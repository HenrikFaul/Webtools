...
import type {
  ManifestReplayResponse,
  RequestManifestEntry,
  TrafficImportRequest,
  TrafficImportResponse,
  TrafficSourceMode
} from "@/types/trafficImport";
...
export function TrafficImportLab() {
  ...
  const runtimeEntries = entries.filter((entry) => entry.runtimeObservedStatus === "observed");
  const inferredEntries = entries.filter((entry) => entry.runtimeObservedStatus !== "observed");
    ...
    <section className="row" style={{ gap: 16 }}>
      <div className="card row two">
        ...
        <div style={{ display: "flex", alignItems: "flex-end" }}>
          <button onClick={() => void runCrawl()} disabled={loading || !crawlUrl}>
            {loading ? "Crawling…" : "Run live URL analysis"}
          </button>
        </div>
      </div>
      ...
      <div className="card row two">
        ...
        <div style={{ gridColumn: "1 / -1" }}>
          <button onClick={() => void runImport()} disabled={loading}>
            {loading ? "Importing…" : "Import traffic into manifest"}
          </button>
        </div>
      </div>
        ...
        {runtimeEntries.length ? (
          runtimeEntries.map((entry) => (
            <details key={entry.id}>
              <summary>
                {entry.method} {entry.pathTemplate} · audit: {entry.auditVerdict}
              </summary>
      ...
      <div className="card row two">
        ...
        <label>
          ...
          <input
            value={tokenInjection}
            onChange={(e) => setTokenInjection(e.target.value)}
            placeholder="Token added as Authorization/apikey"
          />
        </label>
        <label style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <input
            ...
            style={{ width: 16 }}
          />
          Chain mode (dependency-first)
        </label>
        <div style={{ gridColumn: "1 / -1" }}>
          <button onClick={() => void runReplay()} disabled={replayLoading || entries.length === 0}>
            {replayLoading ? "Replaying…" : "Replay imported manifest sequentially"}
          </button>
        </div>
      </div>
