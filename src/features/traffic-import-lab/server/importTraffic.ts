...
export function importTraffic(payload: TrafficImportRequest): TrafficImportResponse {
  ...
  if (payload.mode === "runtime_browser") {
    warnings.push("Runtime browser capture stub: provide HAR export for deterministic import in this environment.");
    entries = parseManual(payload.rawInput).map((e) => ({ ...e, sourceMode: "runtime_browser", runtimeObservedStatus: "observed" }));
  }
